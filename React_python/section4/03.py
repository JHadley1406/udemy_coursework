...
class Server:
    class __Server:
        def __init__(self):
            ...
            only_messages = self.messages.filter(lambda msg: msg[0] == 'message').map(lambda msg: msg[1].split(',')).publish()

            def queue_order(msg):
                self.orders.put(Order.from_list(msg))
            only_messages.filter(lambda msg: msg[0] == 'order').map(lambda msg: msg[1:]).subscribe(queue_order)

            def process_order(time):
                try:
                    order = self.orders.get_nowait()
                    print('processing order: {} [{}]'.format(order, order.timestatmp))
                    matching = None
