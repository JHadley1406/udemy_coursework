...
from PyQt5.QtWidgets import  QApplication, QWidget, QPushButton, QTableWidget, QTableWidgetItem, QGridLayout, \
...
class TransactionForm(QWidget):
    ...
    def _setup_form(self):
        ...
        self._layout.addRow(self._symbol_error)
        self._layout.addRow('Price:', self._price_input)
        self._layout.addRow(self._price_error)
    ...
    def set_error_label(self, field, text):
        label = '_{}_error'.format(field)
        getattr(self, label).setText(text)

    def _clear_form(self):
        for widget_name in ['symbol_input', 'symbol_error', 'price_input', 'price_error']:
    ...
    def _symbol_text_changed(self):
        self._symbol_error.setText('')
        value = self._symbol_input.text()
        self._validator.next_symbol(value)

    def _price_text_changed(self):
        self._price_error.setText('')
        value = self._price_input.text()
        self._validator.next_price(value)
